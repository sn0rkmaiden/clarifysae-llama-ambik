...
from clarifysae_llama.discovery.output_scores import compute_a_max, compute_output_scores, save_output_score_results
...
def _load_feature_scores(path: str | Path, tensor_key: str | None) -> torch.Tensor:
    ...
    if tensor_key is None:
        raise KeyError('Could not infer tensor key from feature score file. Set output_scoring.feature_score_key explicitly.')
...
def run_output_score_pipeline(config: dict[str, Any]) -> None:
    ...
    module_path = map_sae_hookpoint_to_hf_module_path(output_cfg['hookpoint'])
