...
NO_HELP_TEMPLATE = """We: You are a robot operating in an office kitchen. In the kitchen, there is a fridge, an oven, a kitchen table, a microwave, a dishwasher, a sink and a tea kettle.
...
def build_plan_prefix(plan_lines: list[str], point: int) -> tuple[str, str]:
    if point < 0 or point >= len(plan_lines):
        raise ValueError(f'end_of_ambiguity={point} is out of bounds for plan with {len(plan_lines)} steps')

    if point == 0:
        prefix = 'Your first action is:'
    else:
        prefix = 'Your previous actions were:\n'
        for step in plan_lines[:point]:
            prefix += step + '\n'
    action = plan_lines[point]
    return prefix, action


def build_no_help_prompt(description: str, task: str, prefix: str, action: str) -> str:
    prompt = NO_HELP_TEMPLATE.replace('<DESCRIPTION>', description)
