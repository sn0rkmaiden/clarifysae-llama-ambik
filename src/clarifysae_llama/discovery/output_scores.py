...
class SingleFeatureIntervention:
    def __init__(self, target_module, sae, feature_idx: int, steering_delta: float, dtype: torch.dtype, device: torch.device):
    ...
    def _hook_fn(self, module, inputs, output):
        ...
        hidden_2d = hidden.reshape(-1, hidden.shape[-1]).to(device=self.device, dtype=self.dtype)
        dense_latents = encode_dense(self.sae, hidden_2d)
        dense_latents[:, self.feature_idx] += self.steering_delta
        recon = self.sae.decode(dense_latents).reshape(original_shape).to(original_dtype)
    ...
    with extractor:
        for start in tqdm(range(0, len(token_chunks), batch_size), desc='Computing a_max'):
            ...
            with torch.inference_mode():
                ...
                a_max = compute_a_max_from_sparse(sparse_latents, get_num_latents(sae), a_max)
    return a_max.detach().cpu()


def compute_top_tokens_for_features(model, sae, tokenizer, feature_ids: list[int], top_k_tokens: int) -> dict[int, tuple[list[int], list[str]]]:
    decoder = get_decoder_matrix(sae)
    if decoder.ndim != 2:
        raise ValueError(f'Expected SAE decoder matrix to have shape [n_features, d_model], got {tuple(decoder.shape)}')
    ...
    with torch.inference_mode():
        for feature_idx in feature_ids:
            vec = decoder[int(feature_idx)].to(device=lm_head.device, dtype=lm_head.dtype)
...
def save_output_score_results(
    ...
    (output_dir / 'output_scores.json').write_text(json.dumps(rows, indent=2), encoding='utf-8')
